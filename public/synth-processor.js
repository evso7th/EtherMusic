...
class SynthProcessor extends AudioWorkletProcessor {
    ...
    process(inputs, outputs, parameters) {
        ...
        // This is a simple but effective hard-clip limiter.
        // It's not ideal for audio quality, but it's a guaranteed way to prevent > 1.0 output.
        for (let i = 0; i < outputChannel.length; i++) {
            if (outputChannel[i] > 1.0) {
                outputChannel[i] = 1.0;
            } else if (outputChannel[i] < -1.0) {
                outputChannel[i] = -1.0;
            }
        }

        // Post a debug message on every single block. This is inefficient but guarantees we see the log.
        this.port.postMessage({ type: 'debug', peak: peakLevel.toFixed(4), voices: this.voices.size });
