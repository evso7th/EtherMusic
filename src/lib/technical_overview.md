...
**Вывод:** `PolySynth` не подходит для приложений, где требуется высокая производительность и отзывчивость. Мы решили использовать индивидуальные моносинтезаторы.

### Акт 2: Идиотское решение — Новый синтезатор на каждую ноту

**Проблема:** Наша первая попытка реализовать игру на моносинтезаторах была катастрофической. Мы создавали **новый экземпляр `Tone.Synth` на КАЖДУЮ ноту**, а после её окончания — уничтожали его.
*   **Последствия:** Это было еще хуже, чем `PolySynth`. Постоянное создание и уничтожение объектов приводило к чудовищной нагрузке на сборщик мусора (garbage collector) и вызывало еще больше щелчков и задержек.

**Вывод:** Создавать и уничтожать аудио-компоненты в реальном времени — это архитектурный тупик. Ресурсы нужно переиспользовать.

### Акт 3: Архитектурная битва — Армия моносинтезаторов в пуле

**Решение:** Мы полностью отказались от создания синтезаторов на лету в пользу архитектуры **пула (pool) независимых моносинтезаторов (`Tone.Synth`)**, которая является стандартом в профессиональных аудио-приложениях.

**Как это работает:**
1.  **Инициализация:** При запуске `AudioEngine` создает и заранее инициализирует фиксированное количество одноголосых синтезаторов (`Voice` в нашей терминологии) для каждой партии.
2.  **Распределение голосов:** Когда требуется извлечь ноту, движок находит свободный синтезатор в пуле, "захватывает" его, и он начинает звучать. После окончания ноты он освобождается и возвращается в пул для повторного использования.
...
### Акт 4: Новый враг — Тирания Главного Потока
...
### Акт 5: Финальное решение — Изоляция через Web Worker
...
*   **`AudioEngine.ts`:** "Сердце" приложения. Управляет пулами синтезаторов, всеми каналами (`Tone.Channel`), эффектами (`Tone.Reverb`, `Tone.Delay`) и воспроизведением звуков как от ручной игры, так и от команд из Worker'а.
*   **`autopilot-worker.ts`:** "Мозг" приложения. Живет в отдельном потоке. Содержит всю логику для генерации музыкальных стилей ('Ambient', 'Space' и т.д.).
...
## 5. Замечание о размере проекта

В процессе разработки можно заметить, что папка проекта (в частности, директория `node_modules`) занимает значительный объем дискового пространства (сотни мегабайт). **Это абсолютно нормально для современных веб-проектов.**

Эта папка содержит все инструменты, необходимые для разработки: компилятор TypeScript, сам фреймворк Next.js, библиотеки и их зависимости. **Конечный пользователь никогда не загружает этот объем.**

При выполнении команды `next build` (или `npm run build`), Next.js собирает **оптимизированную, статическую версию приложения** в папку `out`. Именно эта папка, размер которой составляет всего несколько мегабайт, и является финальным продуктом, который развертывается на хостинге.

## 6. Планы на будущее

*   **Таймер сна (Sleep Timer):** Возможность установить таймер, по истечении которого музыка плавно затихнет и остановится.
*   **Система пресетов:** Возможность сохранять и загружать полные конфигурации приложения: настройки инструментов, громкости, эффектов, темпа и стиля автопилота.
